<template>
  <div>
    <div class="batch-crowd"  :class="{'valid-error' :isError }">
      <div class="icon-div">
        <img class="crowd-icon" src="@/assets/lfCanvas/batchCrowd.svg" alt="">
      </div>
      <div class="desc-div">
        <!-- {{ modeProperties?.nodeName }} -->
        <p class="title">{{ crowdData?.nodeName ||'批量人群包' }}</p>
        <p class="title-1">人群规模：<span>{{ crowdData?.crowdSum || crowdData?.selectSum ||'未选择' }}</span></p>
