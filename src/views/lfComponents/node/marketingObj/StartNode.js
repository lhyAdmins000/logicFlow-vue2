...
// 开始节点规则  只能连接人群
const ruleList = ['SINGLE_CROWD', 'BATCH_CROWD', 'RULE_CROWD'];
...
class StartNodeModel extends RectResize.model {
  initNodeData(data) {
    ...
    this.sourceRules = [
      selfValidation,
      {
        message: '开始节点的后置节点只允许连接人群类型',
        validate: (_, targetNode) => ruleList.includes(targetNode.type),
      },
      sourceNodeWithMultipleValid('开始节点后置只能连接一个节点', { graphModel: this.graphModel, id: this.id }),
      // {
...
// 自定义圆角边框
class StartNodeView extends RectResize.view {
  getShape() {
