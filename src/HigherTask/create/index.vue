<template>
  <div class="higher-create">
    <div class="header">
      <div class="bread">
        <div class="first">任务中心</div>
        <div class="line">/</div>
        <div class="other">{{ canvasType === 'edit'?'编辑画布任务': canvasType === 'detail'?'查看画布任务':'创建画布任务'}}</div>
      </div>
    </div>
...
import { getHigerTaskDetail } from '@/api/higherTask.js';
// eslint-disable-next-line
...
// eslint-disable-next-line
// import Menu from './lfComponents/tools/menu.js';
// 单个人群包
...
// 智能扩散
// import SpreadNodeDialog from './components/computeComp/SpreadNodeDialog.vue';
// 高能排序
// import SortNodeDialog from './components/computeComp/SortNodeDialog.vue';
...
// 不可编辑状态集合
import NodeEnum, { canEdit, canNotEdit } from './NodeEnum';
// 需要弹窗的节点type集合
...
export default {
  ...
  computed: {
    ...
    copyId() {
      return this.$router.history.current.query.copyId;
    },
    groupId() {
  ...
  methods: {
    ...
    initLogicFlow() {
      ...
      lf.adapterOut = function (logicFlowData) {
        ...
        logicFlowData.nodes.forEach((node) => {
          node.typeValue = NodeEnum[node.type];
        });
      ...
      this.lf = lf;
      if (this.canvasId || this.copyId) {
        getHigerTaskDetail(this.canvasId || this.copyId).then((res) => {
          console.log(res);
          if (res.code === 2000) {
            this.taskDetail = {
              ...res.data,
              canvasName: this.canvasType === 'copy' ? `${res.data.canvasName}(1)` : res.data.canvasName,
              canvasStatus: this.canvasType === 'copy' ? '' : res.data.canvasStatus,
            };
            // 渲染
            const { nodes, edges, canvasStatus } = this.taskDetail;
            lf.render({ nodes, edges });
            lf.extension.miniMap.show();
            // 不可编辑  非复制执行
            if (this.canvasType !== 'copy' && canNotEdit.includes(canvasStatus?.toString())) {
              this.handleDisable();
            }
          }
        }).finally(() => {
    ...
    handleEnable(id) {
      ...
      this.loading = true;
      // 查询详情 重新渲染
      getHigerTaskDetail(id).then((res) => {
          console.log(res);
          if (res.code === 2000) {
            ...
            // 不可编辑
            if (canNotEdit.includes(canvasStatus?.toString())) {
              this.handleDisable();
            }
          }
        }).finally(() => {
    ...
    handleDisable() {
      ...
      // 设置当前画布状态为不可用
      if (!this.taskDetail.canvasStatus || canEdit.includes(this.taskDetail.canvasStatus?.toString())) {
        this.taskDetail.canvasStatus = 1; // 1 模拟执行中
      }
      // eslint-disable-next-line
...
.content{
  .lf-mini-map{
    ...
    .lf-mini-map-header,.lf-mini-map-close{
      display: none;
    }
  }
  .lf-line-text{
