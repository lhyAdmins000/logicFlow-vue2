  ...
  <div>
    <div class="goal-crowd" :class="{'valid-error' :isError }">
      ...
      <div class="desc-div">
        ...
        <p class="title-1">目标规模：<span>{{ crowdData?.crowdSum ||'-' }}</span></p>
        <p class="title-1">发送上限：<span>{{ crowdData?.selectSum ||'-' }}</span></p>
...
.goal-crowd{
  ...
  .icon-div{
    ...
    margin-top: 5px;
    background: linear-gradient(180deg, #409EFF 0%, rgba(64, 158, 255, 0.8) 100%);
    .crowd-icon{
      width: 20px;
      height: 20px;
      margin-top: 2px;
