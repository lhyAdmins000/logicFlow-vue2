...
import Vue from 'vue';
// eslint-disable-next-line
import BatchCrowd from './vComp/BatchCrowd.vue';
...
class BatchCrowdView extends HtmlNode {
  constructor(props) {
    super(props);
...
class BatchCrowdModel extends HtmlNodeModel {
  setAttributes() {
    this.width = width;
    this.height = height;
  ...
  initNodeData(data) {
    ...
    this.sourceRules = [
      selfValidation,
      sourceNodeWithMultipleValid('', { graphModel: this.graphModel, id: this.id }),
      {
        message: '批量人群包后置节点必须为目标节点或计算节点！',
        validate: (_, targetNode) => afterNodeType.includes(targetNode.type),
      },
    ];
    // 不能作为目标
    this.targetRules.push({
      message: '批量人群包节点不能设置前置节点！',
      validate: () => false,
    });
  }
...
// class BatchCrowdModel extends RectResize.model {
//   initNodeData(data) {
//     super.initNodeData(data);
//     // 校验规则
//     // 作为来源时
//     this.sourceRules = [
//       selfValidation,
//       afterNodeWithGoalCrowdValid('批量人群包的后置节点必须为目标节点'),
//     ];
...
// // 自定义圆角边框
// class BatchCrowdView extends RectResize.view {
//   getShape() {
