<template>
  <div>
    <div class="rule-crowd" :class="{'valid-error' :isError }">
      <div class="icon-div">
        <img class="crowd-icon" src="@/assets/lfCanvas/ruleCrowd.svg" alt="">
      </div>
      <div class="desc-div">
        <!-- {{ modeProperties?.nodeName }} -->
        <p class="title">{{ crowdData?.nodeName ||'人群规则' }}</p>
        <p class="title-1">人群规模：<span>{{ crowdData?.crowdSum ||'-' }}</span></p>
      </div>
