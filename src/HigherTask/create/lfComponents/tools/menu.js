...
class Menu {
  ...
  render(lf, container) {
    ...
    this.__menuDOM.addEventListener('click', (event) => {
      ...
      if (Array.from(target.classList).indexOf('lf-menu-item') > -1) {
        // 如果点击区域在菜单项内
        (target).onclickCallback(this.__currentData);
        // 点击后隐藏menu
    ...
    this.lf.on('node:contextmenu', ({ data }) => {
      ...
      this.showMenu(model.x + model.width / 2 + 5, model.y - model.height / 2, menuList, {
        width: model.width,
        height: model.height,
        // clientX: e.clientX,
        // clientY: e.clientY,
    ...
    this.lf.on('blank:click,graph:transform', () => {
      this.__menuDOM.style.display = 'none';
    });
  }
  ...
  showMenu(x, y, menuList, options) {
    ...
    // 节点坐标转为html坐标
    const [hx, hy] = graphModel.transformModel.CanvasPointToHtmlPoint([x, y]);
    console.log(hx, hy);

    menu.style.top = `${hy}px`;
    menu.style.left = `${hx}px`;
