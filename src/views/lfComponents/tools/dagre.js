...
export default class Dagre {
  ...
  layout(option = {}) {
    ...
    console.log(nodes);
    const sortNodes = this.handleSortNode(nodes);
    // console.log('handleSortNode', sortNodes, nodes);
  ...
  // 递归对节点进行排序
  handleSortNode(needSortNode, result = []) {
    needSortNode.forEach((item) => {
      // 获取下一级节点
      const targetNodes = this.lf.getNodeOutgoingNode(item.id);
      if (!result.find((resultItem) => resultItem.id === item.id)) {
        result.push(item);
      }
      this.handleSortNode(targetNodes, result);
    });
    return result;
  }
